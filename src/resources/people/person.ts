...
import { NoteResource } from "./note.js";
...
export class PersonResource {
  ...
  /**
   * Access notes for this person
   */
  notes() {
    if (!this.personId) {
      throw new Error("Person ID is required for note operations");
    }
    return new NoteResource(this.client, this.personId);
  }
}
